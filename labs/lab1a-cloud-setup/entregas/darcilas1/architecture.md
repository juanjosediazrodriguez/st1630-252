...
**Estudiante:** Daniel Arcila Salazar · _(completar correo @eafit.edu.co)_
...
flowchart TD
    ...
    S3 --> G
```
![alt text](diagrama_arquitectura_lab1.jpg)
...
La mayor duda estuvo en adaptar el laboratorio al Sandbox, especialmente por las restricciones de IAM y EMR Studio. Consulté a Cha# Arquitectura — Lab 1a
...
|---|---|---|
| Configuración inicial y troubleshooting de AWS CLI | Sí | Se usó ChatGPT para identificar errores de configuración, credenciales temporales y diferencias entre PowerShell, WSL y Git Bash. |
...
| Configuración IAM | Parcial | Se analizó con ChatGPT el error `AccessDenied` y se verificaron los roles disponibles, pero la elección se basó en las restricciones reales del Sandbox. |
| Creación y troubleshooting de EMR | Sí | Se utilizó ChatGPT para adaptar `create_emr.sh`, reutilizar los roles existentes y agregar Livy/JupyterEnterpriseGateway. |
...
| Captura y análisis del DAG en Spark UI | Sí | Se usó ChatGPT para localizar la consulta Parquet y verificar que el DAG incluyera un nodo `Exchange`. |
